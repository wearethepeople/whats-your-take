...
// form — same shape as the "bring the table to your town" limiter, kept as
// its own module (rather than shared) so a burst on one form never blocks
// the other. In-memory, per-IP, never persisted: the IP is read only to
// make an allow/reject decision for a few minutes, never logged, never
// written to the database, gone on every restart.
